var restas = {
    ...
    preguntaRepetida: function () {
  	...
  	generoPregunta: function() {
        do {
            this.a[this.contadorDePreguntas] = Math.floor((Math.random() * (this.maximo-this.minimo)) + this.minimo + 1);
            do {
                this.b[this.contadorDePreguntas] = Math.floor((Math.random() * (this.maximo-this.minimo+1)) + this.minimo);
            } while (this.b[this.contadorDePreguntas]>=this.a[this.contadorDePreguntas])
    ...
    iniciar: function (minimo, maximo) {
        ...
        $('#boton').on('click', function(){restas.comenzar(minimo, maximo)});          
        $('#ingreso').on('keypress', function(e){restas.verificar(e)});   
    ...
    verificar: function (e) {
        ...
        if (this.contadorDePreguntas<this.cantidadDePreguntas) {
            setTimeout(function(){ restas.preguntar() }, this.tiempoEntrePreguntas*1000)
        }
        else {
            setTimeout(function(){ restas.mostrarFinal() }, this.tiempoEntrePreguntas*1000)
        }
    },
